...
\begin{enumerate}[leftmargin=*, label=\textbf{C\arabic*}.]
  ...
  \item We provide an open-source Python implementation with CPU-parallel
    and Rust-accelerated kernels, and a thorough complexity analysis
...
(with the convention $\phi_e(x_j) = 0$ when $x_j = 0$ and $e < 0$).  The
...
\begin{equation}
  ...
  \label{eq:spending}
\end{equation}
so that $\sum_{t=1}^\infty q_t = Q$.  Compute the \emph{Knockoff+} threshold
\begin{equation}
  \tau_t \;=\;
  \min\!\left\{
    \omega > 0 \;:\;
...
$\hat\Thetab_k$ in place of $\Thetab_k^*$.  By Lemma~6 of \cite{buhlmann2011statistics}
(GraphLasso minimax bound), under Assumption~\ref{asm:bounded} and
$n \gtrsim \log p$:
\begin{equation}
...
dictionary has $\tilde p = pm$ columns.
...
The Python reference implementation uses NumPy and scikit-learn.  Two
optional accelerated kernels are available:
\begin{itemize}
  \item \textbf{C++ kernel}: Vectorised dictionary expansion and knockoff
    sampling, compiled with OpenMP for multi-core execution.
  \item \textbf{Rust kernel}: A safe, zero-copy Rust extension (via PyO3)
    for the inner knockoff-sampling loop, providing $3$--$5\times$ speedups
    on large $p$.
\end{itemize}
All three kernels produce identical numerical results; the Python kernel
serves as the reference for correctness testing.
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Default sweep}: $p=5$, $d=2$, $k \in \{2,3,4\}$,
...
and 0.422; PolyOMP ranges from 0.111 to 0.511; SparsePolySTLSQ ranges
...
and $d=2$ (Table~\ref{tab:degree_k}), though at the cost of FDR~$= 0.389$
in the same setting.  PolyOMP achieves F1~$= 0.933$ at $n=500$, with
FDR~$= 0.111$.
...
implementation with Rust and C++ acceleration.
...
recall but incur FDR between 0.1 and 0.7, making them unsuitable for
