#!/usr/bin/env python3
"""Enhanced Experiment: Settings where IC-Knock-Poly should excel

Key advantages of IC-Knock-Poly:
1. High-dimensional settings (p > n) - where FDR control is crucial
2. Complex true models (larger k) - where iterative selection helps
3. High noise - where robustness matters
4. Correlated features - where GMM modeling helps
...
RESULTS_FILE = os.path.join(OUTPUT_DIR, f'final_enhanced_{TIMESTAMP}.json')
N_TRIALS = 3
N_VAL = 100
...
def main():
    print("="*80)
    print("ENHANCED EXPERIMENT - High-Dimensional & Complex Settings")
    print("="*80)
    print("Focus: p > n, larger k, higher noise, more correlation")
    print("="*80)
    
    # HDLSS (High-Dimensional Low-Sample-Size) experiment
    # Focus on LARGER k (complex models) with moderate n and p
    # p=10, d=3 -> 60 features, so need n >= 30-50 to learn properly
    configs = []
    
    # Scenario 1: Moderate n with moderately large k
    # n=30-50, p=8, k=7,10,12 (complex but learnable models)
    for n in [30, 40, 50]:
        for p in [8, 10]:
            for k in [7, 10, 12]:  # moderately large k - complex but learnable
                for noise in [0.5]:
                    for d in [2, 3]:
                        if k <= 2 * d * p:
                            configs.append((n, p, k, d, noise))
    
    # Scenario 2: Larger n with larger k
    for n in [40, 50, 60]:
        for p in [8, 10]:
            for k in [12, 15, 18]:  # larger but still learnable
                for noise in [0.5]:
                    for d in [2, 3]:
                        if k <= 2 * d * p:
                            configs.append((n, p, k, d, noise))
    
    # Scenario 3: True HDLSS with moderate n and p > n
    for n in [20, 25, 30]:
        for p in [15, 20]:  # p > n/2 but p features expanded to 60-120
            for k in [5, 7]:  # moderate k
                for noise in [0.5]:
                    for d in [2]:  # use d=2 to keep expansion manageable
                        if k <= 2 * d * p:
                            configs.append((n, p, k, d, noise))
    
    # Scenario 4: Standard comparison (small k)
    for n in [30, 50]:
        for p in [8]:
            for k in [3, 5]:  # baseline comparison
    ...
    print(f"  n values: {sorted(set(c[0] for c in configs))}")
    print(f"  p values: {sorted(set(c[1] for c in configs))}")
    print(f"  k values: {sorted(set(c[2] for c in configs))}")
    print(f"  noise values: {sorted(set(c[4] for c in configs))}")
    print(f"  degree values: {sorted(set(c[3] for c in configs))}")
